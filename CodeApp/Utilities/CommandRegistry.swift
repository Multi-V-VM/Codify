...
class CommandRegistry {
    ...
    private var commandHandlers:
        [String: (Int32, UnsafeMutablePointer<UnsafeMutablePointer<Int8>?>?) -> Int32] = [:]

    private init() {}

    func registerCommand(
        _ name: String,
        handler: @escaping (Int32, UnsafeMutablePointer<UnsafeMutablePointer<Int8>?>?) -> Int32
    ) {
    ...
    func handleCommand(
        _ name: String, argc: Int32, argv: UnsafeMutablePointer<UnsafeMutablePointer<Int8>?>?
    ) -> Int32? {
...
public func custom_command_handler(
