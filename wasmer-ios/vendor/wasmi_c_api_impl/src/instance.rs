...
use wasmi::{Extern, ExternType, Instance};
...
pub unsafe extern "C" fn wasm_instance_new(
    ...
    let supplied = (*imports)
        ...
        .collect::<Vec<_>>();
    // Wasmer's ModuleInfo polyfill groups imports by index space, while Wasmi
    // consumes them in their original declaration order. Preserve ordering
    // within each kind and rebuild the declaration order expected by Wasmi.
    let mut funcs = VecDeque::new();
    let mut tables = VecDeque::new();
    let mut memories = VecDeque::new();
    let mut globals = VecDeque::new();
    for import in supplied {
        match import {
            Extern::Func(_) => funcs.push_back(import),
            Extern::Table(_) => tables.push_back(import),
            Extern::Memory(_) => memories.push_back(import),
            Extern::Global(_) => globals.push_back(import),
        }
    }
    let imports = wasm_module
        ...
        .filter_map(|import| match import.ty() {
            ExternType::Func(_) => funcs.pop_front(),
            ExternType::Table(_) => tables.pop_front(),
            ExternType::Memory(_) => memories.pop_front(),
            ExternType::Global(_) => globals.pop_front(),
        })
        .collect::<Box<[_]>>();
