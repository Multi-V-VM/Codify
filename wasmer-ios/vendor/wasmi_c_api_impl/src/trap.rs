...
pub extern "C" fn wasm_trap_new(
    ...
    // Wasmer's Rust host-function bridge passes wasm_name_t buffers without a
    // trailing NUL. The wasm-c-api type is length-delimited, so accept both
    // representations instead of panicking across an extern "C" boundary.
...
pub extern "C" fn wasm_trap_message(trap: &wasm_trap_t, out: &mut wasm_message_t) {
    ...
    // Wasmer's Wasmi backend uses this prefix to recover the boxed Rust host
    // error (notably WasiError::Exit) from the C-API trap.
    buffer.extend_from_slice(format!("Exception: {}", trap.error).as_bytes());
